...
tar-archive, and files which are not tied to the disk image (see
``assumptions'' as defined in Section \ref{sec:proving}). The first needs an
Inode Index to serve as an identifier; the second needs the parent archive's
file identifier as well as an offset within the archive; the final will make
use of a (potentially partial) list of file data, {\tt list (N*Byte)}, to
identify itself.
...
lemmas apply for partial images, they also apply for ``whole'' disk image.
...
We also use hypotheses to account for a transformations that would normally
...
The latter step could be automated using {\tt Fetch} as described in the
previous section.
...
version. For example, have have shown that boolean equality between Files
