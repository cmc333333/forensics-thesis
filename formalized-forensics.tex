...
Section 8 will investigate whether or not Borland's definition is sufficient
...
defined in Section 3.) The former needs an Inode Index to serve as an
identifier; the second needs the archive's file identifier as well as an
offset within the archive; the final will use {\tt ByteData} (i.e.  the file
data) to identify.
...
function (described in Section 4.???); for the Mock file system, it means
delegating to the contained {\tt ByteData}; Tar file processing will be
...
Section 4.???'s discussion of the {\tt Fetch} construction for more.
...
parameter in the definition. As we will see in Section 5.???, this parameter
percolates to the top of the proof; we will effectively run the function
via an external program in a preprocessing step.
...
will cover in sections 5 and 6. Before diving in, however, we should clarify
the relationships between some of the concepts we are proposing; this is the
