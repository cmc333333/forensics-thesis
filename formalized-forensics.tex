...
associated with that file. That file is created depending on the results of a
deletion check and an attempt to parse the associated Inode structure. If
either of those computations fails, we want the entire computation to fail, so
we use a monadic approach. 

\begin{lstlisting}
Function parseFileFromInodeIndex (disk: Disk) (inodeIndex: Z) : Exc File :=
  (Ext2IsDeletedInode inodeIndex disk) flatmap{ fun deleted =>
    (ext2_inode_from disk inodeIndex) map{ fun inode =>
      mkFile
