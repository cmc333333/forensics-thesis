...
\section{Tar Structures and Computations+}
\label{sec:tar}

Our definition of rootkit installation made use of {\tt Tar.parseFileNames}, a
function which retrieves included file names from a tar archive. As with the
Ext2 file system, retrieving these file names requires a discussion of the tar
file format. Unlike our Ext2 studies, which described the layout of a file
system on a disk image, tar files are constructed within the context of a
particular {\tt File}. This means that we will only access bytes on the disk
through the {\tt file @[ offset | disk ]} construct; we need not worry about
underlying file system mechanics.
...
\begin{lstlisting}
Definition parseFirstFileNameAndSize (tar: File) (offset: N) 
  (disk: Disk)
  : @Fetch (string*N) :=
  let firstHundredBytes := 
    map (fetchByte tar.(fileId) disk) 
  ...
  (* File Size is encoded in octal, represented as ASCII 
     characters. It begins at offset 124 and runs for 11 
     characters *)
  (seq_list (fetchByte tar.(fileId) disk) (offset + 124) 11) 
