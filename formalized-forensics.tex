...
file system, it means searching through the contained list of offsets; Tar
file processing will be explained in Section \ref{sec:tar}.
...
This very simple proof clearly hides a great deal of complexity. Why is there
a hypothesis about {\tt gunzip}? What are {\tt honeynet\_image\_partial} and
{\tt gunzipped\_partial}? How does {\tt borland\_reflection} work? Where did
{\tt file23} and the \\{\tt maliciousFileName}s come from?  While we will
answer each of these questions in detail, it is important to recognize that
writing the individual proof is very straight forward (and could easily be
automated).  This plays a heavy role in the future of this type of research,
...
{\tt honeynet\_image\_partial} has the type, {\tt Disk}, which is used to
represent a random-access disk throughout our definitions and proofs.
Technically, it is function from {\tt N} to {\tt Fetch Byte}, acting as a
byte-retrieval mechanism; give a {\tt Disk} an offset, and it will respond
with the byte value at the offset requested or an error code. The {\tt
ByteData} type is simply an alias of {\tt Disk}, but provides a more semantic
name.
...
The {\tt gunzip} hypothesis should serve as a model for introducing the {\em
original} disk image. Currently, we prove several lemmas about a {\em partial}
disk image, but we have not, technically, shown that those lemmas apply to the
entire, original disk image. Ideally, we would create a top-level variable to
represent the whole disk image (as we did representing the ``gunzipping''
operation). We would then define a relationship between partial disk images
and the never-instantiated whole disk image such that if we were to prove our
lemmas about the partial image, it would imply the same lemma regarding the
whole image. This argument is easy to make, could conceivable be difficult to
implement.

Indeed, an external (to Coq) proof checker still needs to be written to verify
these hypotheses. Such a proof checker need not be as complete as Coq, and
should certainly avoid the computational limits Coq imposes. Ideally, such a
checker would have a very small, hand-verified code base; it only needs to
verify the types of hypotheses that Coq cannot. As we have seen with this
research, this means (at minimum) the ability to read from the {\em whole}
disk image.
