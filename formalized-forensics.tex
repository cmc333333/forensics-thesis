...
reasonable when both proving a lemma (via compute) and verifying it (via Qed).
Further, adding non-relevant bytes to the disk image appears to have little
effect on type-checking (i.e. verifying the proof). While adding extra bytes
does not appear to affect the computation steps in the Borland proof, it does
appear to have negative consequences to the Lee lemma. Of course, the more we
are trying to prove, the longer we can expect our computations to run.
